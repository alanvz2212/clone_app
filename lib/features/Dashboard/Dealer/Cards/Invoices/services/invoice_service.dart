...
class InvoiceService {
  static const String _baseUrl = 'http://devapi.abm4trades.com';
  ...
  Future<InvoiceResponse> getCustomerSales(int customerId) async {
    try {
      final uri = Uri.parse('$_baseUrl/Inventory/Sales/GetCustomerSales?customerId=$customerId');
  ...
  Future<List<InvoiceModel>> getCustomerInvoices(int customerId) async {
    ...
    print('Response success: ${response.success}'); // Debug print
    print('Response message: ${response.message}'); // Debug print
    print('Response data length: ${response.data.length}'); // Debug print
    if (response.success) {
      return response.data;
    } else {
      throw Exception(response.message.isNotEmpty ? response.message : 'Failed to fetch invoices');
    }
  }
